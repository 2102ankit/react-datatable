...
function applyTextFilter(products, field, op, value) {
  ...
  return products.filter(product => {
    ...
    switch (op) {
      ...
      case 'begins_with': return fieldValue.startsWith(value);
      case 'ends_with': return fieldValue.endsWith(value);
      case 'equals': return fieldValue === value;
      case 'not_equals': return fieldValue !== value;
      case 'is_blank': return !fieldValue;
      case 'is_not_blank': return !!fieldValue;
      default: return true;
    }
  });
}
...
function applyNumberFilter(products, field, op, value, value2 = null) {
  ...
  return products.filter(product => {
    ...
    switch (op) {
      ...
      case 'neq': return numFieldValue !== numValue;
    ...
    Object.keys(req.query).forEach(key => {
      if (key.includes('_')) {
        ...
        if (suffix === 'like') {
          ...
          filteredProducts = applyTextFilter(filteredProducts, field, suffix.replace('_', 'is_'), '');
        }
      }
    });
